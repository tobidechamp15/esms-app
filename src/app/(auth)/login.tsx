...
export default function LoginScreen() {
  ...
  async function handleVerifyOtp() {
    ...
    try {
      ...
      if (isPinSet) {
        ...
        // No PIN set on this device — complete login directly
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      {step === 'phone' && (
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
          <View className="flex-1 px-6 pt-8">
            <Text className="text-2xl font-bold text-navy mb-1">Welcome Back</Text>
          ...
          <View className="px-6 pb-6 gap-3">
            <Button label="Continue" onPress={handleSendOtp} disabled={phone.length < 10} loading={loading} />
            <Text
      ...
      {step === 'otp' && (
        <View className="flex-1 px-6 pt-8">
          <Text className="text-2xl font-bold text-navy mb-1">Verify Your Number</Text>
          ...
          {error ? <Text className="text-danger text-sm mt-4">{error}</Text> : null}
          <View className="flex-1" />
          <View className="gap-4">
            <Button label="Verify Code" onPress={handleVerifyOtp} disabled={otp.length < 6} loading={loading} />
            <NumPad onPress={(d) => { if (otp.length < 6) { setOtp(p => p + d); setError(''); } }} onDelete={() => setOtp(p => p.slice(0, -1))} />
          </View>
        </View>
      )}
      ...
      {step === 'pin' && (
        <View className="flex-1 px-6 pt-8">
          ...
          {error ? <Text className="text-danger text-sm mt-4">{error}</Text> : null}
          <View className="flex-1" />
          <NumPad onPress={handleVerifyPin} onDelete={() => { setPin(p => p.slice(0, -1)); setError(''); }} />
        </View>
      )}
    </SafeAreaView>
  );
