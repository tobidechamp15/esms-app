...
export default function VisitsScreen() {
  ...
  const { visits, isLoading, isFetching, pagination, fetchVisits, fetchAllVisits } = useVisitStore();
  ...
  const load = (replace = true) => {
    ...
    return isSecurityOrAdmin ? fetchAllVisits(params, replace) : fetchVisits(params, replace);
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          isLoading ? (
            <ActivityIndicator color="#3C9FFE" style={{ marginTop: Spacing.six }} />
          ) : (
            <EmptyState
              emoji="📋"
        ...
        ListFooterComponent={
          isFetching ? <ActivityIndicator color="#3C9FFE" style={{ marginVertical: Spacing.three }} /> : null
        }
        renderItem={({ item }) => (
          <VisitCard
            visit={item}
...
const styles = StyleSheet.create({
  ...
  addBtn: { backgroundColor: '#3C9FFE', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 8 },
  addBtnText: { color: '#fff', fontSize: 14, fontWeight: '700' },
  searchContainer: { marginHorizontal: Spacing.four, marginTop: Spacing.three, borderRadius: Spacing.two, paddingHorizontal: Spacing.three },
  searchInput: { paddingVertical: 10, fontSize: 15 },
  filterList: { paddingHorizontal: Spacing.four, paddingVertical: Spacing.two, gap: Spacing.one },
