...
const STATUSES: { key: "submitted" | "under_review" | "resolved"; label: string }[] = [
...
export default function ReportDetailScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface">
      ...
      {isLoading ? (
        <View className="flex-1 items-center justify-center"><ActivityIndicator color="#1B4FD8" /></View>
      ) : isError || !report ? (
        <View className="flex-1 items-center justify-center px-6">
          <Text className="text-danger text-sm mb-2">Couldn't load this report</Text>
        ...
        <ScrollView className="flex-1 px-6" contentContainerClassName="pb-10">
          <View className="bg-white border border-border rounded-2xl p-5 mt-4">
            <Text className="text-lg font-bold text-navy mb-1">{report.subject}</Text>
            <Text className="text-sm text-muted">{report.address}</Text>
            {report.resident ? (
              <Text className="text-xs text-muted mt-3">
                From {report.resident.firstName} {report.resident.lastName} · {report.resident.phone}
              </Text>
            ) : null}
          ...
          <Text className="text-sm font-semibold text-navy mt-6 mb-3">Update Status</Text>
          {STATUSES.map((s) => {
            ...
            return (
              <Pressable
                ...
                className={`h-12 rounded-2xl items-center justify-center mb-2 border ${
                  active ? "bg-[#084BA3] border-[#084BA3]" : "bg-white border-border"
                }`}
              >
                <Text className={`text-sm font-medium ${active ? "text-white" : "text-navy"}`}>{s.label}</Text>
              </Pressable>
            );
          })}
        </ScrollView>
