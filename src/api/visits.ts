...
export async function revokeVisit(id: string): Promise<Visit> {
  const { data } = await apiClient.patch<ApiResponse<Visit>>(VISIT_ENDPOINTS.REVOKE(id));
  return data.data;
}

export async function getTodayStats(): Promise<TodayStats> {
  const { data } = await apiClient.get<ApiResponse<TodayStats>>(VISIT_ENDPOINTS.STATS_TODAY);
  return data.data;
}
